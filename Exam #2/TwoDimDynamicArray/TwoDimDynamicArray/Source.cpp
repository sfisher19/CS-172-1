...
int** fillArray(unsigned int numRows, unsigned int numCols)
{
	...
	My2DArray = new int*[numRows];	//Dynamically allocating the array with size numRows		
	...
	for (int i = 0; i < numRows; i++) {
		...
		//and 2) assign it to the current row of your array.
			...
			My2DArray[i] = new int[numCols]; //Registers the values input by the user
											//And puts them in the newly allocated array
...
void deleteArray(int** A, int size)
{
	...
	{
		delete [] A[i];//As the number of rows increases going through the for loop,
				//The array will delete every row until there are no rows needing to be deleted
	}

	delete [] A; //This deletes the existing array
}